...
unsafe fn disable_power_throttling(verbose: bool) -> Result<(), String> {
    unsafe {
        ...
        let power_throttling = ProcessPowerThrottling {
...
unsafe fn set_timer_resolution(desired_resolution: u32, verbose: bool) -> Result<u32, String> {
    unsafe {
        // Load ntdll.dll
        ...
        if status != 0 {
            return Err(format!(
                "NtSetTimerResolution failed with status: 0x{:08X}",
                status
            ));
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.resolution < 5000 || args.resolution > 156_000 {
        eprintln!(
            "⚠️  Warning: Resolution {} is outside typical range (5000-156000 units)",
            args.resolution
        );
    ...
    let current_resolution = unsafe { set_timer_resolution(args.resolution, args.verbose)? };
